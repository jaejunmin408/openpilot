...
부호 컨벤션:
  - Alpamayo(y=LEFT, yaw=CCW) → 내부 ego(y=RIGHT, yaw=CW). parse 시 flip.
  - 내부 y=RIGHT 는 openpilot device frame(camera.py:155 "y -> right",
    laneLineMeta.rightY=+1.8) 과 일치.
  - openpilot desiredCurvature 는 RIGHT 양수 컨벤션
    (honda/carcontroller.py:142 "positive when steering right" + apply_torque =
     -limited_torque ⇒ actuators.torque LEFT 양수, latcontrol_torque return =
     -output_torque ⇒ setpoint = desired_curv*v² 가 RIGHT 양수).
  - 표준 pure pursuit κ = +2·lateral_right/L_d² (별도 반전 없음).
"""
import json
import math
import socket
...
def parse_action_packet(data: bytes):
    ...
    # Alpamayo(y=LEFT, yaw=CCW) → 내부(y=RIGHT, yaw=CW). openpilot device frame
    # 및 desiredCurvature(RIGHT 양수) 와 일치시키기 위해 부호 반전.
...
def pure_pursuit_curvature(pkt, s_now):
    ...
    부호: 내부 y=RIGHT, z=DOWN, 양의 회전=CW. lateral_right>0 → 우회전 →
          openpilot RIGHT-positive κ 와 부호 일치 (반전 없음).
    """
    s_max = float(pkt['s'][-1])
    L_d = PP_LOOKAHEAD_M
    ...
    # (x_fwd, y_right, z_down) frame 에서 perp_right = R(+90° z_down) · T = (-Ty, Tx)
...
def slice_path_current_ego(pkt, s_now):
    ...
    원점=path[s_now], +x=접선, +y=접선 RIGHT (z=DOWN, CW positive).
    """
    s_arr = pkt['s']
    if s_now >= s_arr[-1]:
